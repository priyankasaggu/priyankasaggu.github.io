...
I want to check if and how the data is persisted in Etcd.
...
1. Exec inside the etcd pod in the `kube-system` namespace of your kubernetes cluster — `kubectl exec -it -n kube-system etcd-kep-4595-cluster-control-plane -- /bin/sh`
2. Create alias — `alias e="etcdctl --endpoints 127.0.0.1:2379 --cert=/etc/kubernetes/pki/etcd/server.crt --key=/etc/kubernetes/pki/etcd/server.key --cacert=/etc/kubernetes/pki/etcd/ca.crt"`
3. Access the data — `e get --prefix /registry/apiextensions.k8s.io/`
